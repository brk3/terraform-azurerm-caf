locals {
  ...
  combined_objects_azurerm_firewalls                              = merge(tomap({ (local.client_config.landingzone_key) = local.firewall_modules }), try(var.remote_objects.azurerm_firewalls, {}))
  ...
  combined_objects_networking                                     = merge(tomap({ (local.client_config.landingzone_key) = local.networking_modules }), try(var.remote_objects.vnets, {}))
  ...
  combined_objects_public_ip_addresses                            = merge(tomap({ (local.client_config.landingzone_key) = local.public_ip_addresses_modules }), try(var.remote_objects.public_ip_addresses, {}))
    ...
    tomap(
      {
        (local.client_config.landingzone_key) = merge(
          ...
          { ("hub_subscription") = { id = data.azurerm_subscription.hub.id } }
        )
      }
    ),
